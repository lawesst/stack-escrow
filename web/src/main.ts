...
async function refreshWalletAddress(): Promise<void> {
  const config = currentConfig();
  const result = await request('stx_getAddresses', { network: config.network });
  const stxAddress = result.addresses.find(addr => addr.symbol === 'STX')?.address ?? result.addresses[0]?.address ?? '';

  state.walletAddress = stxAddress;
  must<HTMLSpanElement>('#wallet-address').textContent = stxAddress || 'Not connected';
}
    ...
    <div class="shell">
      ...
      <section class="panel">
        ...
        <div class="actions">
          ...
          <button id="refresh-wallet" type="button">Refresh Address</button>
          <span class="muted">Wallet: <code id="wallet-address">Not connected</code></span>
        </div>
      </section>
  ...
  must<HTMLButtonElement>('#connect-wallet').addEventListener('click', async () => {
    try {
      ...
      await showConnect({
        ...
        onCancel: () => setStatus('Wallet connection cancelled', 'warn'),
      });
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Wallet connect failed', 'err');
    }
  });

  must<HTMLButtonElement>('#refresh-wallet').addEventListener('click', async () => {
    try {
      await refreshWalletAddress();
      setStatus('Wallet address refreshed', 'ok');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Failed to refresh wallet address', 'err');
    }
  });
